program main
  ...
  do t=1,tt
     ...
     if(t.le.3)then
        ...
        ! elseif(mod(t,4).eq.1)then
        !    m%mdrr(t) = max(m%mdr(t),m%mdrr(t-1))
        ! elseif(mod(t,4).eq.2)then
        !    m%mdrr(t-1) = max(m%mdr(t-1),m%mdrr(t-2))
        !    m%mdrr(t  ) = max(m%mdr(t  ),m%mdrr(t-1))           
        ! elseif(mod(t,4).eq.3)then
        !    m%mdrr(t-2) = max(m%mdr(t-2),m%mdrr(t-3))
        !    m%mdrr(t-1) = max(m%mdr(t-1),m%mdrr(t-2))
        !    m%mdrr(t  ) = max(m%mdr(t  ),m%mdrr(t-1))
        ! end if
        ! !print*,t,m%mdrr(t-3:t)
     end if
     
           
           
     !m%mdrr(t-2) = m%mdrr(t-3)*(0.96d0+m%mrandom(t)*(1.04d0-0.96d0))
     ...
     if(t.gt.3)then
        ...
        !print*,t,mod(t,3)
        m%mdrr(t-3:t) = m%mdr( t-3:t)*((1d0-coeff)+m%mrandom(t-3)*(2d0*coeff))
        !m%mdrr(t-3:t) = m%mdr( t-3:t)*(0.96d0+m%mrandom(t-3)*(1.04d0-0.96d0))
        
        call recov_masses(t-3,t,m%ppi_0(t-3:t),vvacc(t-3:t),m%mdrr(t-3:t),m%nse(t-3:t-1),s0,i0,c0,r0)
        m%msb(t) = s0
        m%mib(t) = i0
        m%mcb(t) = c0
        m%mrb(t) = r0
     ...
     if(t.gt.1) then
        write(1,'(i15,20f20.10)')t,m%mie(t)*pop,m%mse(t),m%mde(t)*pop,pop*(m%mde(t)-m%mde(t-1)),m%ppie(t),m%nse(t)/n_bar, &
             aux,m%mie(t),m%mib(t),m%delay(t),m%mdrr(t)*pop,pop*(m%mdrr(t)-m%mdrr(t-1)),m%nagg(t)/n_bar
     end if
     mdr_1 = m%mdr(t)
  end do
  close(1)
